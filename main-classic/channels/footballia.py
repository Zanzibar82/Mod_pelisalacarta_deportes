...
def play(item):
    logger.info("pelisalacarta.channels.footballia play")
    itemlist = []
    data = scrapertools.cachePage(item.url)

    if "Este partido no está disponible temporalmente por motivos legales" in data: return itemlist
    scrapedurl = scrapertools.find_single_match(data, 'file: "([^"]+)')
    itemlist.append(Item(channel=__channel__, title=item.title, action="play", server="directo", url=scrapedurl, thumbnail=item.thumbnail, folder=False))
    return itemlist
